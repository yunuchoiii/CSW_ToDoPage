...
const BoardCard = ({ board }: BoardCardProps) => {
  ...
  const handleClickBoard = () => {
    ...
    const rgbaColor = getRGBAFromHex(board.color);
    const startColor = rgbaColor.replace(", 1)", ", 0.1)");
    const endColor = rgbaColor.replace(", 1)", ", 0.5)");
    changeBackgroundColor(isActive ? "" : `linear-gradient(150deg, ${startColor} 0%, ${endColor} 100%)`);
  };
  ...
  return <section 
    ...
    style={{
      borderColor: activeBoardId === board.id ? board.color : "transparent",
    }}
  ...
  >
    <div>
      <div className="flex items-start justify-between">
        ...
        >
          <img src="https://www.svgrepo.com/show/509384/more-vertical.svg" alt="more" className="w-4 h-4" />
        </button>
      </div>
